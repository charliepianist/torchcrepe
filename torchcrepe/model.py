...
class Crepe(torch.nn.Module):
    ...
    def __init__(self, model='full'):
        ...
        self.dropout = torch.nn.Dropout(0.25) # Dropout addition
    ...
    def layer(self, x, conv, batch_norm, padding=(0, 0, 31, 32)):
        ...
        x = F.relu(x)
        x = batch_norm(x)
        x = self.dropout(x) # Dropout addition
...
# Model with additional layer normalization at end, so KL divergence can be used

class CrepeKL(torch.nn.Module):
    """Crepe model definition"""
    ...
    def __init__(self, model='full'):
        ...
        self.softmax = torch.nn.LogSoftmax(dim=1) # Normalization addition
...
# Model without final layer
class CrepeSmall(torch.nn.Module):
    """Crepe model definition"""
...
# Crepe with extra layer before FC

class CrepeLarge(torch.nn.Module):
    """Crepe model definition"""
    ...
    def __init__(self, model='full'):
        ...
        # Model-specific layer parameters
        if model == 'full':
            in_channels = [1, 1024, 128, 128, 128, 256, 512]
            out_channels = [1024, 128, 128, 128, 256, 512, 1024]
            self.in_features = 2048
        elif model == 'tiny':
            in_channels = [1, 128, 16, 16, 16, 32]
            out_channels = [128, 16, 16, 16, 32, 64]
        ...
        self.conv7 = torch.nn.Conv2d(
            ...
            stride=strides[6])
        self.conv7_BN = batch_norm_fn(
            num_features=out_channels[6])
    ...
    def forward(self, x, embed=False):
        ...
        # Forward pass through layer six
        x = self.layer(x, self.conv6, self.conv6_BN)
        # Forward pass through layer seven
        x = self.layer(x, self.conv7, self.conv7_BN)
